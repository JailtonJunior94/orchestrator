...
func (r1 *Executor) Execute(tool, skill, projectDir string, args []string, fsys fs.FileSystem) (instruction string, err error) {
	...
	passed, results, err := prerequisites.NewVerifier().Verify(skill, projectDir, fsys)
	if err != nil {
	...
	tokens, limit, ok := metrics.NewCatalog().CheckBudget(content, tool)
	if !ok {
		return "", fmt.Errorf("budget excedido para %q: %d tokens estimados, limite %d", tool, tokens, limit)
	}

	instruction = NewExecutor().buildInstruction(tool, skill, projectDir, args)
	return instruction, nil
}

func (r1 *Executor) buildInstruction(tool, skill, projectDir string, args []string) string {
	extraArgs := ""
	if len(args) > 0 {
