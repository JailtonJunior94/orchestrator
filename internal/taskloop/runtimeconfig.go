...
// defaultACPActivityTimeout é o watchdog default do task-loop ACP (F1): 120s.
...
func BuildRuntimeConfig(resolved config.Runtime) (airuntime.RuntimeConfig, error) {
...
func resolveRuntimeConfig(cwd string, flagsOverrides config.Runtime) (airuntime.RuntimeConfig, error) {
	...
	rc, err := BuildRuntimeConfig(resolved)
	if err != nil {
...
func optionsToConfigOverrides(opts Options) config.Runtime {
	var timeout string
