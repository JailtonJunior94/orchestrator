...
func TestHookContractHasNoProviderIdentifier(t *testing.T) {
	...
	for _, entry := range entries {
		...
		ast.Inspect(file, func(node ast.Node) bool {
			switch n := node.(type) {
			case *ast.Ident:
				failIfForbidden(t, entry.Name(), n.Name)
			case *ast.BasicLit:
				if n.Kind == token.STRING {
					failIfForbidden(t, entry.Name(), n.Value)
				}
			}
			return true
		})
...
func failIfForbidden(t *testing.T, filename, text string) {
	t.Helper()
