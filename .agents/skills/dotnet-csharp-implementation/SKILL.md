...
**Padroes C# (inline)**
- Habilitar Nullable Reference Types (`<Nullable>enable</Nullable>`) e tratar warnings de nulidade
  (CS86xx) como erros em projetos novos. Usar `required` em propriedades obrigatorias na inicializacao.
- Propagar `CancellationToken` ate a borda de IO em toda operacao assincrona. Nunca usar `.Result`
  ou `.Wait()` — causam deadlock e bloqueiam o thread pool.
- Usar `record` (ou `record struct`) para DTOs, value objects e domain events imutaveis; `class` para
  entidades de dominio com identidade e mutabilidade controlada.
- Retornar `Result<T>` (ou equivalente) em fronteiras de Application/Domain para fluxos esperados
  (not found, validacao). Reservar exceptions para falhas inesperadas de infraestrutura.
- Injetar `TimeProvider` para tempo testavel em vez de `DateTime.UtcNow` hardcoded.
- Resolver `HttpClient` sempre via `IHttpClientFactory` — nunca instanciar com `new`.
- Formatar com `dotnet format`. Usar `dotnet build` + `dotnet test` como gate minimo.
