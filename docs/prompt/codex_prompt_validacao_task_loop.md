...
```text
Valide o `task-loop` deste repositorio de forma efetiva, rigorosa, economica e enxuta em `Codex`.

Objetivo:
- executar uma validacao real do fluxo do `task-loop`
- validar obrigatoriamente em `Codex`
...
2. Executar um `dry-run` do `task-loop` com `Codex`.
...
- Fixei o formato de saida para garantir evidencia objetiva da execucao em `Codex`.
