...
```text
Valide o `task-loop` deste repositorio de forma efetiva, rigorosa, economica e enxuta em `Claude Code`.

Objetivo:
- executar uma validacao real do fluxo do `task-loop`
- validar obrigatoriamente em `Claude Code`
...
2. Executar um `dry-run` do `task-loop` com `Claude Code`.
...
- Fixei o formato de saida para garantir evidencia objetiva da execucao em `Claude Code`.
