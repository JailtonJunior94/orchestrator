...
while IFS= read -r line; do
  ...
  [[ ${#cells[@]} -lt 6 ]] && continue
  tid=$(echo "${cells[1]}" | xargs)
  status=$(echo "${cells[3]}" | xargs)
  deps=$(echo "${cells[4]}" | xargs)
  parallel=$(echo "${cells[5]}" | xargs)
...
if [[ ${#IDS[@]} -gt 1 ]]; then
  ...
  if [[ ${#SORTED[@]} -gt 1 ]]; then
    ...
    if [[ $actual -lt $expected ]]; then
      ...
      echo "WARN F29: gaps na numeracao: ${MISSING[*]}. Confirme intencional ou corrija tasks.md." >&2
      warnings=$((warnings+1))
    fi
  fi
fi
  ...
  for dep in "${DEP_LIST[@]}"; do
    ...
    if [[ "$dep" =~ ^([a-z][a-z0-9_-]*)/([0-9]+\.[0-9]+)$ ]]; then
      ...
      if [[ -f "$ext_prd_md" ]]; then
        ...
        if [[ -n "$ext_hash" ]]; then
          current_hash=$(sha256sum "$ext_prd_md" | awk '{print $1}')
