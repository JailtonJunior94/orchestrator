...
1. Input: slug curto, `prd-<slug>`, ou path. Normalizar para `${AI_TASKS_ROOT:-tasks}/${AI_PRD_PREFIX:-prd-}<slug>/`.
2. `source scripts/lib/check-invocation-depth.sh` — parar se profundidade exceder.
3. `ai-spec skills --verify` — `blocked` se drift.
4. Confirmar `prd.md`, `techspec.md`, `tasks.md` → `needs_input` se faltar.
5. `ai-spec check-spec-drift tasks/prd-<slug>/tasks.md` → `blocked` se RF não coberto.
6. **Validação git opt-in (F35, ativar via `AI_VALIDATE_GIT_HISTORY=1`)**: para cada tarefa `done`, extrair `DiffSHA` do report e validar `git cat-file -e <sha>`. SHA ausente (revert/rewrite) → `needs_input: tarefa <id> done mas DiffSHA <sha> não está no git log — possível revert. (a) re-execute, (b) edite status, (c) cancele`.
...
1. Ler `tasks.md`. Parsear cada linha com regex canônicos (gerados por `create-tasks` v1.4+):
   ...
   - `dependências`: `^(—|(\w[\w-]*\/)?\d+\.\d+(,\s*(\w[\w-]*\/)?\d+\.\d+)*)$`. Cross-PRD via prefixo `<slug>/`. Resolução em 5 passos:
     ...
     3. **Spec-hash do PRD referenciado (F18)**: extrair `spec-hash-prd` do header e comparar com `sha256sum tasks/prd-<slug>/prd.md`. Divergente → `blocked: cross-PRD <slug> tem spec drift; rode 'ai-spec check-spec-drift' e re-execute aquele PRD primeiro`.
   ...
   - `paralelizável`: `^(—|Não|Com\s+\d+\.\d+(,\s*\d+\.\d+)*)$`. `—`/`Não` = sequencial; `Com X.0[, ...]` = paralelo. Fora = `failed: malformed Paralelizável on <id>`. Sem parsing tolerante (case-sensitive).
...
- "`export AI_INVOCATION_DEPTH=0` + sourcing `scripts/lib/check-invocation-depth.sh`."
...
1. **Formato canônico**: bloco com exatamente `status`, `report_path`, `summary`. Texto livre/diff → `failed: contract violation`.
...
4. **Consistência tasks.md** para `done`: re-ler tasks.md, confirmar status atualizado. Divergente → `failed: status drift`.
...
| Copilot CLI | Custom Agents ([ref](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/create-custom-agents-for-cli)). Auto-descobre `.agents/skills/`, `.claude/skills/`, `.github/skills/` | `.github/agents/task-executor.agent.md` | `/fleet` ou multi-session | n/d |
...
`tasks/prd-<slug>/` resolve para `${AI_TASKS_ROOT:-tasks}/${AI_PRD_PREFIX:-prd-}<slug>/`. Configurar em `.claude/config.yaml`/`.agents/config.yaml` (`tasks_root`, `prd_prefix`, `task_timeout_seconds`). Vars exportadas por `scripts/lib/check-invocation-depth.sh`.
