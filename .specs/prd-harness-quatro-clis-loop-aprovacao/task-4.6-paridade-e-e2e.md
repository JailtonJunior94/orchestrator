# Tarefa 4.6: Prova de paridade entre os três caminhos e fluxos E2E

<critical>Ler prd.md e techspec.md desta pasta — sua tarefa será invalidada se você pular</critical>

## Visão Geral

Quarta e última fatia da promoção do loop ao agregado. Com os três caminhos (`RunLoop` em 4.3,
`Service.Execute` em 4.4, `ACPRunner` em 4.5) já consumindo o agregado, esta fatia entrega a
**prova** de que se comportam de forma idêntica — a techspec registra que o repositório já sofreu
caso de unitário verde com integração real falhando, por isso a prova E2E é obrigatória.
...
- A virada do critério estrito **não** acontece aqui (é 5.0). Esta fatia fecha o estágio de
  paridade com a suíte existente verde — o gate de não-regressão mais importante do plano (ADR-001,
  item 4).
...
- [ ] 4.6.1 Teste de paridade: mesmo cenário de entrada produz veredito, motivo canônico e
      estrutura de evidência idênticos em `ACPRunner`, `Service.Execute` e `RunLoop`.
- [ ] 4.6.2 Cinco fluxos E2E com o servidor ACP falso in-process já existente no repositório:
      (a) ciclo que aprova na primeira rodada; (b) ciclo que aprova na terceira após duas correções;
      (c) ciclo que aborta por fingerprint repetida **sem gastar a rodada seguinte**; (d) ciclo que
      aborta por ausência de mudança; (e) ciclo cuja remediação não produz diff.
- [ ] 4.6.3 Teste dedicado: `APPROVED_WITH_REMARKS` realimenta a correção (confirma o
      comportamento fiado em 4.5 ponta a ponta).

## Detalhes de Implementação

Seguir a techspec desta pasta, **"Abordagem de Testes" → "Testes E2E"** (servidor ACP falso
in-process) e a ADR-001 "Plano de Implementação" item 4. `4.3-decomposition-rationale.md` delimita a
...
- `.specs/prd-harness-quatro-clis-loop-aprovacao/techspec.md` — "Abordagem de Testes" → "Testes
  E2E".
