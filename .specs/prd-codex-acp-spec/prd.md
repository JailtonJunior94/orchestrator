...
> **ADR material**: [.specs/adr/013-codex-cli-acp-native.md](../adr/013-codex-cli-acp-native.md)
> **Precedente direto**: F1-Copilot ([.specs/prd-copilot-acp-spec/](../prd-copilot-acp-spec/)) — gate-keeper de generalização runtime
...
- **HU-01**: Como **operador da CLI**, quero invocar `ai-spec-harness task-loop --tool codex --runtime acp .specs/prd-minha-feature` e ver o harness conectar via ACP, produzir `events.jsonl` linha-a-linha e gerar `execution_report.md` no padrão Claude/Copilot.
...
- **HU-06**: Como **operador legado**, quero que `ai-spec-harness task-loop --tool codex .specs/...` (sem `--runtime=acp`) continue funcionando como hoje (via `codexInvoker` invocando `codex exec --yolo`), com aviso de depreciação em log.
...
`internal/runtime/probe/probe.go:21-24` adiciona `"codex": ".specs/adr/013-codex-cli-acp-native.md"`. Mensagem de erro de probe Codex referencia ADR-013 explicitamente.
...
- **RF-14**: `internal/runtime/probe/probe.go:21-24` ganha `"codex": ".specs/adr/013-codex-cli-acp-native.md"` em `adrByID`.
...
3. **CLI — erro de launcher unavailable** (mensagem RF-14 generalizada):
   ```
   codex-acp não encontrado. Install codex-acp; OR install @zed-industries/codex-acp@0.14.0 via npm; OR use --runtime=legacy. Veja .specs/adr/013-codex-cli-acp-native.md
   ```
