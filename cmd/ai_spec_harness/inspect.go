...
Exemplos:
  ...
  ai-spec-harness inspect . --complexity=standard`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			focusPaths := newFlagHelper().parseFocusPaths(inspectFocusPaths)
	...
	cmd.Flags().StringVar(&inspectFocusPaths, "focus-paths", "", "Prioriza deteccao de toolchain proximo desses arquivos, separados por virgula (util em monorepos). Alternativa: env FOCUS_PATHS")
	cmd.Flags().BoolVar(&inspectBrief, "brief", false, "Carrega apenas blocos TL;DR das referencias (economiza tokens)")
	cmd.Flags().StringVar(&inspectComplexity, "complexity", "", "Filtra referencias por nivel de complexidade: trivial, standard, complex (default: complex)")
	return cmd
}
