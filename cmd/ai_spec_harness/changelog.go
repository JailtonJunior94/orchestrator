...
Exemplos:
  ...
  ai-spec-harness changelog ./meu-repo --version 1.3.0 --dry-run`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := newFlagHelper().requireFlag(cmd, "version", "ai-spec-harness changelog . --version 1.3.0"); err != nil {
				return err
			}
			...
			section, err := changelog.NewGenerator().GenerateChangelog(repoPath, changelogVersion, date, "")
			if err != nil {
				return err
			}
			...
			if err := changelog.NewGenerator().UpdateChangelog(changelogPath, section); err != nil {
	...
	cmd.Flags().StringVar(&changelogVersion, "version", "", "Versao SemVer sem prefixo v (obrigatorio)")
	cmd.Flags().BoolVar(&changelogDryRun, "dry-run", false, "Imprime no stdout sem modificar o arquivo")
	return cmd
}
