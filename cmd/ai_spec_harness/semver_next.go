...
func newSemverNextCmd() *cobra.Command {
...
Exemplos:
  ...
  ai-spec-harness semver-next ./meu-repo --format json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			d, err := semver.NewService().Evaluate(repoPath)
			if err != nil {
				return err
			}
	...
	cmd.Flags().StringVar(&semverNextFormat, "format", "text", "Formato de saida: text ou json")
	return cmd
}
